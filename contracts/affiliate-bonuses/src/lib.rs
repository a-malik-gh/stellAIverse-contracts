...
impl AffiliateBonuses {
    ...
    pub fn init_contract(env: Env, admin_addr: Address, config: CommissionConfig) -> Result<(), ContractError> {
        ...
        if config.commission_rate_bps > 5000 { // Max 50%
            return Err(ContractError::InvalidAgentId);
        }
        if config.min_volume_for_commission < 0 || config.payout_threshold < 0 || config.max_commission_per_user < 0 {
            return Err(ContractError::InvalidAgentId);
        }
        
        env.storage().instance().set(&Symbol::new(&env, ADMIN_KEY), &admin_addr);
        env.storage().instance().set(&Symbol::new(&env, "config"), &config);
        env.storage().instance().set(&Symbol::new(&env, "affiliate_counter"), &0u64);
        
        Ok(())
    }

    /// Register as an affiliate with a unique referral code.
    pub fn register_affiliate(env: Env, affiliate: Address, referral_code: String) -> Result<(), ContractError> {
        ...
        env.events().publish(
            (Symbol::new(&env, "affiliate"), Symbol::new(&env, "registered")),
    ...
    pub fn record_referral(env: Env, referred_user: Address, referral_code: String) -> Result<(), ContractError> {
        referred_user.require_auth();
        
        // Find affiliate by referral code
        let code_key = (Symbol::new(&env, "code"), referral_code.clone());
        let affiliate: Address = env.storage().instance().get(&code_key)
    ...
    pub fn track_volume(env: Env, caller: Address, user: Address, volume: i128, transaction_id: String) -> Result<(), ContractError> {
        caller.require_auth();
        ...
        let affiliate: Address = env.storage().instance().get(&user_referral_key)
            .ok_or(ContractError::InvalidAgentId)?; // User not referred
        
        let config: CommissionConfig = env.storage().instance().get(&Symbol::new(&env, "config"))
        ...
        if env.storage().instance().has(&volume_key) {
        ...
        let mut referral_record: ReferralRecord = env.storage().instance().get(&referral_key).unwrap();
        ...
        if commission_amount > 0 && referral_record.total_volume >= config.min_volume_for_commission {
            ...
            // Check max commission per user
            let user_commission_key = (Symbol::new(&env, "user_commission"), affiliate.clone());
            let mut user_total_commission: i128 = env.storage().instance().get(&user_commission_key).unwrap_or(0);
            ...
            if user_total_commission + commission_amount <= config.max_commission_per_user {
                ...
                let affiliate_key = (Symbol::new(&env, "affiliate"), affiliate.clone());
                let mut info: AffiliateInfo = env.storage().instance().get(&affiliate_key).unwrap();
                info.total_volume_generated += volume;
                info.total_commissions_earned += commission_amount;
                info.last_activity_at = now;
                env.storage().instance().set(&affiliate_key, &info);
    ...
    pub fn claim_commissions(env: Env, affiliate: Address) -> Result<i128, ContractError> {
        ...
        env.events().publish(
            (Symbol::new(&env, "commission"), Symbol::new(&env, "claimed")),
    ...
    pub fn get_affiliate_info(env: Env, affiliate: Address) -> Result<AffiliateInfo, ContractError> {
    ...
    pub fn get_affiliate_by_code(env: Env, referral_code: String) -> Result<Address, ContractError> {
    ...
    pub fn get_top_affiliates(env: Env, limit: u32) -> Vec<AffiliateInfo> {
        let affiliate_counter: u64 = env.storage().instance().get(&Symbol::new(&env, "affiliate_counter")).unwrap_or(0);
        let mut affiliates = Vec::new(&env);
        
        // This is a simplified approach - in production, you'd want a more efficient ranking system
        for i in 1..=affiliate_counter {
            if affiliates.len() >= limit as u32 {
                break;
            }
            
            let affiliate_addr = Address::generate(&env); // This would need proper tracking
            let key = (Symbol::new(&env, "affiliate"), affiliate_addr);
            if let Ok(info) = env.storage().instance().get::<_, AffiliateInfo>(&key) {
                affiliates.push_back(info);
            }
        }
