...
impl MultisigWaitlist {
    ...
    pub fn init_contract(env: Env, admin_addr: Address, config: MultisigConfig) -> Result<(), ContractError> {
        ...
        if config.min_signers == 0 || config.max_signers < config.min_signers || config.max_signers > 100 {
        ...
        env.storage().instance().set(&Symbol::new(&env, "granted_counter"), &0u64);
    ...
    pub fn join_waitlist(
        ...
        let config: MultisigConfig = env.storage().instance().get(&Symbol::new(&env, "config"))
        ...
        let entry = MultisigWaitlistEntry {
            ...
            status: if config.auto_grant_enabled { WaitlistStatus::Granted } else { WaitlistStatus::Pending },
            required_signers,
            wallet_name: wallet_name.clone(),
            use_case: use_case.clone(),
            granted_at: if config.auto_grant_enabled { Some(env.ledger().timestamp()) } else { None },
            wallet_address: None,
        ...
        env.events().publish(
            (Symbol::new(&env, "multisig_waitlist"), Symbol::new(&env, "joined")),
    ...
    pub fn approve_users(env: Env, admin: Address, users: Vec<Address>, notes: String) -> Result<u64, ContractError> {
        ...
        let granted_counter: u64 = env.storage().instance().get(&Symbol::new(&env, "granted_counter")).unwrap_or(0);
        ...
        for user in users.iter() {
            ...
            let user_key = (Symbol::new(&env, "user"), user.clone());
            if let Ok(mut entry) = env.storage().instance().get::<_, MultisigWaitlistEntry>(&user_key) {
                if entry.status == WaitlistStatus::Pending {
                    ...
                    env.events().publish(
                        (Symbol::new(&env, "multisig_waitlist"), Symbol::new(&env, "approved")),
                        (user, new_batch_id),
                    );
                }
            }
    ...
    pub fn grant_access(env: Env, admin: Address, user: Address, wallet_address: Address) -> Result<(), ContractError> {
        admin::verify_admin(&env, &admin)?;
        
        let user_key = (Symbol::new(&env, "user"), user.clone());
        let mut entry: MultisigWaitlistEntry = env.storage().instance().get(&user_key)
        ...
        env.storage().instance().set(&Symbol::new(&env, "granted_counter"), &(granted_counter + 1));
        
        env.events().publish(
            (Symbol::new(&env, "multisig_waitlist"), Symbol::new(&env, "granted")),
            (user, wallet_address),
        );
    ...
    pub fn decline_access(env: Env, user: Address) -> Result<(), ContractError> {
        ...
        env.events().publish(
            (Symbol::new(&env, "multisig_waitlist"), Symbol::new(&env, "declined")),
            (user, entry.position),
        );
    ...
    pub fn remove_user(env: Env, admin: Address, user: Address) -> Result<(), ContractError> {
        ...
        env.events().publish(
            (Symbol::new(&env, "multisig_waitlist"), Symbol::new(&env, "removed")),
            (user, entry.position),
        );
    ...
    pub fn get_waitlist_entry(env: Env, user: Address) -> Result<MultisigWaitlistEntry, ContractError> {
    ...
    pub fn get_waitlist_stats(env: Env) -> Result<MultisigWaitlistStats, ContractError> {
        ...
        for position in 1..=waitlist_counter {
            ...
            if let Some(user) = env.storage().instance().get::<_, Address>(&position_key) {
                ...
                if let Ok(entry) = env.storage().instance().get::<_, MultisigWaitlistEntry>(&user_key) {
                    match entry.status {
...
mod tests {
    ...
    fn test_multisig_waitlist_flow() {
        ...
        let approved = client.approve_users(&admin, users, &String::from_str(&env, "Batch 1 approval"));
